...
export const PromptEditor = () => {
   ...
   useEffect(() => {
      if (editingState) {
         // When starting to edit a message, load its content into the input
         setPrompt(editingState.content);
      } else {
         // When editing is cancelled or finished, clear the input
         setPrompt("");
      }
   }, [editingState]);
   ...
   useEffect(() => {
      ...
      const handleClickOutside = (event: MouseEvent) => {
         if (templateAnchorRef.current && !templateAnchorRef.current.contains(event.target as Node)) {
         ...
         <div className="mx-auto w-full max-w-4xl">
            {editingState ? (
               <div className="mb-2 flex items-center justify-between rounded-xl border border-amber-400 bg-amber-50 px-4 py-2 text-xs text-amber-900 dark:border-amber-500 dark:bg-amber-500/10 dark:text-amber-100">
                  ...
                  <button type="button" onClick={cancelEditingMessage} className="font-medium underline">
                     Cancel
                  </button>
               </div>
            ) : null}
               ...
               <div ref={templateAnchorRef} className="relative flex items-center gap-2 rounded-3xl border border-gray-300 bg-white px-4 py-2 shadow-sm dark:border-gray-700 dark:bg-slate-900">
                  ...
                  {showTemplates && (
                     <div className="absolute bottom-full left-0 z-20 mb-2 w-72 rounded-xl border border-gray-200 bg-white p-2 text-xs shadow-lg dark:border-gray-700 dark:bg-slate-950">
                        <div className="mb-1 px-1 text-[16px] font-medium text-gray-700 dark:text-gray-300">Prompt templates</div>
                        <div className="max-h-64 space-y-1 overflow-y-auto">
                           {TEMPLATES.map((template) => (
                              ...
                              >
                                 ...
                                 <div className="mt-0.5 text-[10px] text-gray-500 dark:text-gray-400">{template.category}</div>
                              </button>
                           ))}
                        </div>
                     </div>
                  ...
                  <textarea
                     ...
                     className="prompt-textarea flex-1 resize-none bg-transparent text-sm text-gray-900 outline-none placeholder:text-gray-500 leading-[1.4] py-1 min-h-6 max-h-40 overflow-y-auto dark:text-gray-100"
                     rows={1}
                     onKeyDown={(e) => {
